...
/* global require, exports, console, process, device, cordova, module, L, angular, _, jQuery, window, document, Image, Backbone, syncedStorageGUID */
...
(function() {
	...
	var PRODUCTION_SERVER = {
			...
			WS : "/api/v2/ws/socket.io"
		},
		DEBUG_SERVER = {
			MAIN : "http://localhost:3200",
			BASEWS : 'http://localhost:3201',
			WS : "/socket.io/"
		},
		SERVER = PRODUCTION_SERVER; // DEBUG_SERVER; // PRODUCTION_SERVER; // DEBUG_SERVER; // DEBUG_SERVER; // PRODUCTION; // DEBUG_SERVER_URL, // PRODUCTION_SERVER_URL;

	angular
		.module('lifecourse').factory('utils',function($injector) {
			var DEBUG=0, INFO=1, LOG=2, WARN=3, ERROR=4, DEBUG_LEVEL = DEBUG,
				jQ = jQuery,
				utils = {
					REMOTE_SERVER_URL:SERVER.MAIN,
					REMOTE_WS_BASE:SERVER.BASEWS,
					REMOTE_WS_PATH:SERVER.WS,
					...
					IMAGE_TYPES : {
						'image/png':true,
						'image/jpg':true // add more here
					},
					isOnboarded : function(profile) { 
						return this.getFactory('storage').getProfile().then(function(profile) { 
							var obModel = utils.getModel(profile,'onboarded');
							return obModel.get('status') === true;
						});
					},
					definedEqual : function(o1,o2) { 
						var o1keys = _(o1).keys().filter(function(k) { return o1[k] !== undefined && o1[k] !== null; }),
							o2keys = _(o2).keys().filter(function(k) { return o2[k] !== undefined && o2[k] !== null; });

						if (o1keys.length !== o2keys.length) { return false; }

						var o1pick = _(o1).pick(o1keys),
							o2pick = _(o2).pick(o2keys);
	                ...
	                getFactory:function(factoryName) { 
	                    // var injector = angular.injector(['afinity']);
	                ...
	                sendFeedback: function (data) {
	                    ...
	                    var cont = 	function() { 
	                    	console.log("sendFeedback - sending ", data, ' to ', that.REMOTE_SERVER_URL);
		                    jQuery.ajax({
		                        url: that.REMOTE_SERVER_URL + "feedback",
		                        type: "POST",
		                        data: data,
		                        success: function (data, textStatus, jqXHR) {
		                            try {
		                                ...
		                                console.log("sendFeedback - success, error caught", e, e.stack);
		                                d.reject(e);
		                            }
		                        },
		                        error: function (e) {
		                            console.log("sendFeedback - error", e, e.stack);
		                            d.reject(e);
		                        }
		                    }); 
		                 };
	                    console.log("sendFeedback - device", typeof device !== 'undefined' ? device : '');
	                    if (typeof device !== 'undefined') { 
	                    	device.getInfo(function(devdata) { 
	                    		data.device = devdata;
	                    		cont();
	                    	});
	                    } else { data.device = { type : 'browser'}; cont(); }
	                    return d;
	                },
					...
					getMetric:function(type, v){
						...
						if(type === 'weight'){
							return (v.stones * 14 + v.pounds) * 0.453592;
						}
						if(type === 'height')
							return (v.feet * 12 + v.inches) * 2.54;
					},
	                ...
	                deprecate:function (inFavourOf, msg) {
	                    return function () {
	                        console.warn(msg);
	                        return inFavourOf.apply(undefined, arguments);
	                    };
	                },
					toRelativeDateString : function(d) {
						...
						return this.DOW_FULL[d.getDay()];
						// return [this.DOW_SHORT[d.getDay()],d.getDate(),this.MON_SHORT[d.getMonth()]].join(' ');
					},
					getModel:function(collection,id,nosync) {
						var storage = $injector.get('storage'),
							existing = collection && collection.get(id);
						// console.info('getModel ', collection, collection.name, id, ' existing ? ', existing !== undefined);
						if (existing !== undefined) { return existing; }
						var newmodel = collection.makeImmediate(id,{},nosync); 
						// newmodel.save(); // we yolo this // collection.create({}, {id:id,nosync:nosync});
						return newmodel;
					},
	                avg:function(l) {
					...
					toISODateString:function(d) { 
						...
						return this.MON_FULL[d.getMonth()] + " " + (d.getDate()) + ", " + d.getFullYear();
						// return d.toLocaleDateString(); // d.toISOString();
					},
					...
					getPlatform:function() { 
						...
						return (window.device && window.device.platform) || undefined;
					},
					dmap: function(L, fn) {
		...
		}).directive('networkError', function() { 
			return {
				restrict:'E', scope:{retry:'='},
				transclude:true,
				replace:true,
				templateUrl:'tmpl/network-error-widget.html'
			};
